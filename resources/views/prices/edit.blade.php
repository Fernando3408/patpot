<form method="POST" action="/precios/{{ $price->id }}">
    ...
    <div class="form-section-header">Editar precio por cliente</div>
    ...
    <div class="form-grid">
        ...
        <div class="form-group">
            ...
            <input id="price_box" type="number" step="1" min="0" name="price_box" class="form-control @error('price_box') is-invalid @enderror" value="{{ old('price_box', floatval($price->price_box)) }}" required>
            @error('price_box')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <input id="offer_price" type="number" step="1" min="0" name="offer_price" class="form-control @error('offer_price') is-invalid @enderror" value="{{ old('offer_price', floatval($price->offer_price ?? '')) }}">
            @error('offer_price')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
